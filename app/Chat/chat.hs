...
u = run $ recordType (Proxy :: Proxy Message)
...
main
 ...
 = do
  ...
  -- Start tasks
  void $
    (mapM (\f -> async $ f subj) $ on Input inputMode ++ on Output outputMode) >>=
    waitAnyCancel
    ...
    outputMode subj =
      runApp def ByType $ \conn
       ...
       ->
        ...
        execStateT
          (runEffect $
           pipeIn conn >-> niceMessage subj >-> for cat (liftIO . putStrLn))
          (ChatState False)
    inputMode subj =
      runApp def ByType $ \conn -> do
       ...
       -- Read lines from the user (stdinLn) and send them out (pipeOut)
        runEffect $
          for
            stdinLn
            (\txt -> unless (null txt) (yield . msg . TextMessage $ txt)) >->
          pipeOut conn
