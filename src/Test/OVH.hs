...
import Test.Types (Test (Test), test)
...
ovhTest :: String -> Test
ovhTest srvName = let Just srv = lookup srvName servers in test ("Test OVH server " ++ srvName) (availableServer srv) (parseRequest (ksURL srv) >>= getURL)

availableServer :: String -> B8.ByteString -> Maybe String
...
isAvailable = any (any ((/= "unavailable") . availability) . datacenters) . filter ((`elem` ["europe", "northAmerica"]) . region)
